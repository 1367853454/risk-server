...
public class StringUtil {
    ...
    public static String dashCase(String capitalize) {
        if (isBlank(capitalize)) {
            return capitalize;
        } else {
            char[] origins = capitalize.toCharArray();
            StringBuilder sb = new StringBuilder();

            for(int i = 0; i < origins.length; ++i) {
                if (Character.isUpperCase(origins[i])) {
                    if (i != 0) {
                        sb.append("-");
                    }

                    sb.append(Character.toLowerCase(origins[i]));
                } else {
                    sb.append(origins[i]);
                }
            }
